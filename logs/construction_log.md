...
| C001 | 2026-07-18 | v1.0 | （待commit回填） | 卷一 cluster A（第346-438行） | 7 | 1 | 0.14 | 6 | 0 | 1 | 0 |  | 无 | V1.0首个正式batch，端到端验证pipeline（locate_candidates→extractor流程→validate_ceu→character-os-updater流程→revision_log）；产出的CEU本身触发1个schema_gap信号（见schema_gaps.md），yield_rate偏低是因为这个cluster主要是场景描写而非独立选择场景，属正常 |
