...
## 2026-07-20 — 卷十一dev盲测复盘：profile v0.8→v0.9四项通用修正 + prediction_protocol v1.0→v1.1两项协议修正

触发：卷十一验证集盲测调试（logs/predictions/vol11_blind.md → vol11_scored.md，0.57/机制0.80/文风3/5；用户指令：基于开发集调试推理逻辑，只允许通用修正）。
修正前→修正后（字段级）：
1. speech_register新增"语体动力学"子字段：高唤起句长骤缩为碎片化短句/结巴/极简应答，铺陈长句仅用于有说服目标的论证（6实例+卷十旁证"遵命，飞鼠大人！"）。change_type: 字段新增
2. psychological_structure"认知加工"deflect guilt形态：追加作用域限定"限安兹对统治/职责过失的自责，社交性道歉不触发"（#10命中vs#26误用的对照）。change_type: 范围限定
3. decision_rules规则9：追加作用域"政务/职责议题；对安兹的私人愿望被拒时零坚持即刻让渡（候选）"（#14误用谏言坚持规则于私愿被拒）。change_type: 范围限定
4. traits新增"翅膀是情绪外显通道"（急切扇动/失落垂落/喜悦伸展，候选，单场景4实例）。change_type: 候选机制新增
协议修正（spec/prediction_protocol.md v1.1，正本变更记录见该文件）：第0步补状态继承规则（#9失败）；新增第0.5步邻接上下文反推功能类型（#3/#20失败vs#25成功对照）。
原因：机制选择0.80与总分0.57的差距定位在文风渲染层与协议执行层，而非模型核心缺口；全部修正通过防过拟合门槛（通用属性/简短逻辑/无场景补丁），场景特定事实（墙上暗格藏行李等）明确不入模型。
round: v2.0
