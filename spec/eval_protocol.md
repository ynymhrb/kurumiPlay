...
版本：v0.3（新增第9节：生成阶段pipeline，心理内核+文学手法两段式生成逻辑）
...
## 9. 生成阶段 pipeline（TODO占位，尚未实际验证，先记录设计避免遗忘）

这一节记录 RolePlay Agent/生成模块该如何消费 `psych_core`/`literary_technique`（见 `CEU_schema.md` v0.7、`attribution_framework.md`），现在只是设计草案，等实际有生成场景可以试跑时再校准，不阻塞当前train阶段。

**两段式生成逻辑**：

```
1. 定位新场景触发的心理内核（可能来自1条或多条CEU的psych_core归纳）
2. 结合新场景的写实尺度/语境，先生成心理内核对应的写实反应（draft）
3. 检索该情绪类型/触发条件是否有已积累的 literary_technique
   （见 characters/<角色>/V<版本>/reaction_stylization.md）
4. 判断是否套用文学化加工（TODO，暂定判断标准，需要实际生成案例验证后重新校准）：
   a. 基调匹配：新场景整体基调是否偏喜剧/戏剧化夸张？严肃向场景默认不套用
   b. 情绪类型匹配：该手法历史上关联的情绪类型，和当前触发情绪是否一致？
   c. 频率合理性：按该手法在对应场合的 p(n)（见 expression_dna_protocol.md 2.4节）加权随机决定是否套用，
      不是满足a/b就无条件触发——避免同类场景每次都写成一个模子
5. 套用 → 输出风格化版本；不套用/场景要求写实 → 保留写实版（draft）
```

`literary_technique` 不是提取阶段就该丢弃的噪音，是可复用的风格化手法，该不该用是生成阶段按场景动态判断的决策，不是提取阶段的一次性取舍——两个版本（写实/风格化）在CEU层面都要保留信息，不能只留一个。
