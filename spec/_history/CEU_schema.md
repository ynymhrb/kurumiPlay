...
版本：v0.7（新增 psych_core / literary_technique 字段，服务生成阶段的心理内核vs文学外壳拆分）
...
underlying_belief:       # 支撑这个选择的深层信念（Mental Model 候选）——回答"为什么"（WHY），信念内容本身
...
psych_core:              # 可选，仅在schema_gap触发或反应幅度明显超出常理判断时按 spec/attribution_framework.md
                         # 走一遍归因分析后填写。回答"这个信念被触发时，一个真人大概会有什么行为反应"——
                         # 去掉原文具体措辞和场景细节的、可迁移到新场景的写实反应模板（不是"为什么"，是"大概会做什么"）
literary_technique:      # 可选，仅当 psych_core 分析判定为"心理燃料+文学外壳"（即心理轴单独无法覆盖原文的
                         # 具体表现形式，如超出真人物理极限的夸张动作）时才填写。记录原文用了什么风格化手法把
                         # 心理内核放大/外化（如"表现主义外化：躯体动作放大情绪"），这是可复用的叙事手法标签，
                         # 积累够样本后进入 characters/<角色>/V<版本>/reaction_stylization.md 的手法库，
                         # 频率标注用 expression_dna_protocol.md 的"特征频率概率模型"
```

## v0.6 → v0.7 变更记录

- 新增 `psych_core`（可选）+ `literary_technique`（可选，依赖psych_core）：源自对4条累积的schema_gap信号
  （YLDB-V3-C-001/E-002/E-004/F-001）的联合讨论——这4条信号的共同点是`choice`/`value_conflict`字段
  预设"当事人清楚自己在权衡什么"这个前提不总成立（反射反应/认知自我说服/表态层vs情绪层/依恋创伤触发）。
  讨论过程中发现更根本的问题：这些证据里，原文的具体表现形式（比如"撞天花板"）本身混杂了两种不同性质
  的信息——**可迁移到新场景的心理真实**，和**只在这个具体场景有效的文学化表现手法**——现有schema把
  两者都塞进`underlying_belief`一个字段，导致生成新场景反应时无法区分"该复用什么、不该字面复制什么"。
  新增两个字段把这两层拆开：`psych_core`回答"信念触发后一个真人大概会做什么"（去场景化的可迁移反应模板），
  `literary_technique`回答"原文用什么风格化手法把这个反应放大/外化"（可复用的叙事技法标签，供生成阶段
  按场景判断是否套用，不是提取阶段就该丢弃的部分）。归因分析方法论见新建的`spec/attribution_framework.md`
  （双轴模型：先假设为真人走心理轴四层，只有心理轴出现"解释赤字"才调用文学轴两层）。
  两个字段都是可选，且要求走过`attribution_framework.md`的分析流程才填写——不是每条CEU都要做这层分析，
  只在schema_gap触发或反应幅度明显超出常理时启用，避免给常规CEU增加不必要的负担。
