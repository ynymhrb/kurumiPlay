...
- `decision_rules.md`：v0.2（新增候选规则7：命令逐字复诵；规则2新增第三层例外+第二组谏言强度对照；规则5新增反例；新增候选规则8：命令提升执行认真度）
...
1. **卷四**——下一步，回来就先做这个（先跑`locate_candidates.py 雅儿贝德 4`生成`_index_vol4.yaml`）
2. 卷五（本轮当前阶段的目标终点）
3. MM4/MM6/MM7 逐条回填三重验证标注（不必卡在卷四/五，可并行积累证据后再做）
4. 卷五处理完后：统一复核累积的7条schema_gap信号

**跨会话恢复进度**：卷一（19/19）、卷二（V0.1遗留复核确认兼容）、卷三（9/9）均已全部完成并commit，`_index_vol3.yaml`的`next_recommended: null`。当前revision_log的"卷三全部完成"条目**尚未commit**（git_ref待回填）。下一步是卷四，本文件"下一步"列表第1项。

---
最后更新：2026-07-18（卷三全部完成），对应 commit `b183c30`（**注意：卷三总结的revision_log条目本身尚未提交**）
