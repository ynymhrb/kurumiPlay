...
CEU (Character Evidence Unit)
 ↓
拆解为多维人物特征：
 - 价值观与追求（Value Hierarchy）
 - 心智模型 / 思维方式（Mental Model）
 - 决策规律（Decision Rules）
 - 关系与权力结构（Relationship Rules）
 - 语言风格（Expression DNA）
 - 环境、年龄段、品格等背景特征
 ↓
Character Runtime Model（以上特征的运行时组合）
 ↓
Generation（言行模拟）
```

关键洞察：人物核心不是"她做过什么选择"的行为记录，而是**驱动这些言行的深层特征组合**。价值冲突时的选择（CEU里的 value_conflict/chosen_value 字段）只是这些特征的一种外显证据，不是特征本身——提取的目标是特征本身（价值观、追求、环境、关系、年龄段、品格……），选择记录只是用来反推特征的原材料。
